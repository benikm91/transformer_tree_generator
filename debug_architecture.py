...
def plot_value(run_fs, dataset: str, key: str, ylabel: str, log_scale=False, ax=None, deltas=None):
    ax = ax or plt.gca()
    colors = {}
    linestyles = {}
    max_epochs = {}
    ...
    for run_name, filenames_f in run_fs.items():
        for depth in range(1, 4):
            filenames = filenames_f(dataset, depth)
            # if key == 'loss_history':
...
def get_filename(task: str, dataset: str, depth: int, run_id: int) -> str:
    if dataset == 'std':
        ...
        return f'out/task/curriculum__decoder-split_l3_h8_e512_wi1_peglobal_learn_te0_ap0.00_ep0.00__{task}_b5_d3__64__actual__{task}_b5_d{depth}__{run_id}.pkl'

plots['Different architecture (bf=5)'] = {
    'fix': lambda dataset, depth: [get_filename('fix', dataset, depth, i) for i in range(3)],
    'lwr': lambda dataset, depth: [get_filename('slwr', dataset, depth, i) for i in range(3)],
    'nwr': lambda dataset, depth: [get_filename('snwr', dataset, depth, i) for i in range(3)],
    'lwor': lambda dataset, depth: [get_filename('slwor', dataset, depth, i) for i in range(3)],
    'nwor': lambda dataset, depth: [get_filename('snwor', dataset, depth, i) for i in range(3)],
}
expected_final_loss = {
...
for plot_name, run_fs in plots.items():
    fig, ax = plt.subplots(2, 2, figsize=(40, 5), sharey='col')
    plot_value(run_fs, 'std', 'tree_acc_history', 'Tree Generation Accuracy', ax=ax[0][0])
    plot_value(run_fs, 'std', 'loss_history', 'Loss', ax=ax[0][1], log_scale=False)
    plot_value(run_fs, 'split', 'tree_acc_history', 'Tree Generation Accuracy', ax=ax[1][0])
    plot_value(run_fs, 'split', 'loss_history', 'Loss', ax=ax[1][1], log_scale=False)
